...
def template_mpc(model, xref, Ts):
    ...
    mpc.set_rterm( a=R[0], alpha=R[1] )

    # Lower bounds on states:
    mpc.bounds['lower','_x', 'x']     = - distance_xy
    mpc.bounds['lower','_x', 'y']     = - distance_xy
    mpc.bounds['lower','_x', 'theta'] = - distance_theta
    mpc.bounds['lower','_x', 'v']     = - vel_v
    mpc.bounds['lower','_x', 'w']     = - vel_w
    # Upper bounds on states
    mpc.bounds['upper','_x', 'x']     = distance_xy
    mpc.bounds['upper','_x', 'y']     = distance_xy
    mpc.bounds['upper','_x', 'theta'] = distance_theta
    mpc.bounds['upper','_x', 'v']     = vel_v
    mpc.bounds['upper','_x', 'w']     = vel_w

    # Lower bounds on inputs:
    mpc.bounds['lower','_u', 'a']     = - vel_v
    mpc.bounds['lower','_u', 'alpha']     = - vel_w
    # Lower bounds on inputs:
    mpc.bounds['upper','_u', 'a']     = vel_v
    mpc.bounds['upper','_u', 'alpha']     = vel_w
